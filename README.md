...
- **Next episode:** when the end credits begin (or in the last seconds, when the credits are unknown or a scene follows them) a card shows the next episode: its code, length, title and a short description, with two buttons. **Next episode** fills up during the countdown when *Autoplay next episode* is on and starts it; **Watch credits** stays in this episode (it says *Cancel* when no credits are playing).
