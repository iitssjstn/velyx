  ...
  return (
    <div className="px-4 pt-8 sm:px-8">
      ...
      {q.error ? (
        ...
        <p className="mt-10 text-muted">{t('searchPage.start')}</p>
      ) : nothing ? (
        <EmptyState icon={<SearchIcon className="size-6" />} title={t('searchPage.nothingFound', { query })}>
          {t('searchPage.nothingFoundHint')}
        </EmptyState>
      ) : r ? (
        <div className="mt-10 space-y-12">
          {r.movies.length > 0 && (
            <section>
              <h2 className="font-display text-xl font-semibold">{t('nav.movies')}</h2>
          ...
          {r.shows.length > 0 && (
            <section>
              <h2 className="font-display text-xl font-semibold">{t('nav.tvShows')}</h2>
              <div className="mt-4 grid grid-cols-[repeat(auto-fill,minmax(8.5rem,1fr))] gap-x-4 gap-y-6 sm:grid-cols-[repeat(auto-fill,minmax(10rem,1fr))]">
                {r.shows.map((s) => (
          ...
          {r.episodes.length > 0 && (
            <section>
              <h2 className="font-display text-xl font-semibold">{t('series.episodes')}</h2>
                  ...
                  <li key={e.id}>
                    <Link to={`/play/episode/${e.id}`} className="flex items-center gap-3 rounded-xl p-2 hover:bg-surface">
                      ...
                      <div className="min-w-0">
                        <p className="truncate font-medium">{e.title ?? t('series.episode', { n: e.episodeNumber })}</p>
