...
function UserForm({ user, isSelf, onDone }: { user?: AdminUser; isSelf: boolean; onDone: () => void }) {
  ...
  const m = useMutation({
    mutationFn: () => {
      if (!user) return api.post('/api/users', { username: form.username.trim(), password: form.password, displayName: form.displayName.trim() || undefined, role: form.role });
      const body: Record<string, unknown> = { displayName: form.displayName.trim() || null };
      if (!isSelf) {
        body.role = form.role;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-muted">Everyone gets their own watch progress and favorites. Administrators manage libraries and users.</p>
