    ...
    <section className="mt-10 first:mt-0">
      <div className="mb-3 flex items-end justify-between px-4 sm:px-8">
        ...
        <div className="flex items-center gap-1">
          ...
          <button type="button" onClick={() => scroll(-1)} className="hidden size-8 place-items-center rounded-full text-muted hover:bg-raised hover:text-ink sm:grid" aria-label={`Scroll ${title} left`}>
            <ChevronLeft className="size-5" />
          </button>
          <button type="button" onClick={() => scroll(1)} className="hidden size-8 place-items-center rounded-full text-muted hover:bg-raised hover:text-ink sm:grid" aria-label={`Scroll ${title} right`}>
