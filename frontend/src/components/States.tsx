...
export function Spinner({ className = '' }: { className?: string }) {
  return <LoaderCircle className={`animate-spin text-accent ${className}`} aria-label="Loading" />;
}
 ...
 */
export function ErrorState({ title = 'Something went wrong.', error, onRetry, fullscreen }: { title?: string; error?: unknown; onRetry?: () => void; fullscreen?: boolean }) {
  ...
  return (
    <div className={`grid place-items-center px-6 ${fullscreen ? 'min-h-dvh' : 'min-h-[50vh]'}`}>
      <div className="max-w-lg text-center">
        ...
        <h1 className="font-display text-3xl font-semibold">{notFound ? 'Not found' : title}</h1>
        <p className="mt-3 text-muted">{error ? errorMessage(error) : 'Try again.'}</p>
        {onRetry && (
        ...
        {isAdmin && detail && (
          <details className="mt-6 text-left">
            <summary className="cursor-pointer text-sm text-muted">Diagnostic details (visible to admins)</summary>
